...
class Pioneer:
  ...
  def __init__(self):
    try:
      ...
      from soar.serial.tools.list_ports import comports
    except ImportError:
    ...
    ports = [p for p,desc,n in comports() if '067b' in n and '2303' in n]
    assert len(ports) > 0, "No available serial ports.  Is the robot plugged in and powered on?"
